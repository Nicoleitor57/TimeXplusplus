...
# from txai.utils.predictors.loss import GSATLoss, ConnectLoss # Usamos las versiones simples
...
#     # --- ¡INSTANCIACIÓN CORREGIDA DE LOS CRITERIOS! ---
#     criterions = {
#         'fidelity': Poly1BCELoss(epsilon=POLY1_EPSILON),
#         'gsat': GSATLoss(r=GSAT_R), # Usamos GSATLoss (simple)
#         'connect': ConnectLoss() # Usamos ConnectLoss (simple)
#     }
...
#     save_path = 'models/my_s4d_explainer_smooth_final.pt'
...
# --- ¡IMPORTACIONES CORREGIDAS! ---
from txai.utils.predictors.loss import GSATLoss, ConnectLoss # Usamos las versiones simples
from visualizar_explicacion import generate_visual_explanation
...
# >> EJECUCIÓN PRINCIPAL <<
# ===================================================================
# ... (todas tus clases y la función train_explainer van aquí arriba) ...

# ===================================================================
# >> FUNCIÓN DE EXPERIMENTO REUTILIZABLE <<
# ===================================================================
def run_experiment(config):
    ...
    optimizer = torch.optim.AdamW(explainer.mask_generator.parameters(), lr=config['LEARNING_RATE'])
    
    criterions = {
        'fidelity': Poly1BCELoss(epsilon=config['POLY1_EPSILON']),
        'gsat': GSATLoss(r=config['GSAT_R']),
        'connect': ConnectLoss()
    }
    
    betas = {'fidelity': config['BETA_FIDELITY'], 'gsat': config['BETA_GSAT'], 'connect': config['BETA_CONNECT']}
    ...
    # Usamos el nombre de la corrida para crear una carpeta única
    save_dir = os.path.join('models', config['run_name'])
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
    save_path = os.path.join(save_dir, 'explainer.pt')

    torch.save(explainer.state_dict(), save_path)
    print(f"\n¡ÉXITO! Explicador para la corrida '{config['run_name']}' guardado en '{save_path}'")
    
    # (Opcional) Guardar también la configuración usada
    ...
    # Prepara la configuración para el script de visualización
    viz_config = config.copy()
    viz_config['explainer_path'] = save_path
    viz_config['s4_model_path'] = PATH_A_TU_MODELO_S4D_ENTRENADO
    viz_config['ecg_path'] = '../../S4D-ECG/exams/19009_lr.dat' # El ECG de ejemplo a explicar
    viz_config['num_leads'] = 12
    viz_config['sampling_hz'] = 100
    viz_config['duration_s'] = 10

    generate_visual_explanation(viz_config)

# Este bloque ahora solo sirve para ejecutar este script de forma individual si quieres
if __name__ == '__main__':
    # Define una configuración por defecto para una corrida manual
    default_config = {
        'run_name': 'manual_run',
        'EPOCHS': 35,
        'LEARNING_RATE': 1e-3,
